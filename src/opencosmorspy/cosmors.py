...
class COSMORS(object):
    ...
    def __init__(self, par='default_turbomole'):
        ...
        if type(par) == str:
            ...
            raise ValueError('Do not know how to handle passed '
                             f'parameterization {par} x')
    ...
    def add_job(self, x, T, refst, x_refst=None):
        ...
        if refst == 'cosmo':
            ...
            for idxm, mol in enumerate(self.enth.mol_lst):
                ...
                if mix_ref is None:
                    self.mix_lst.append(Mixture(x_refst, T,
                                                self.enth, self.comb))
        ...
        if refst == 'reference_mixture':
            ...
            if mix_ref is None:
                self.mix_lst.append(Mixture(x_refst, T,
                                            self.enth, self.comb))
                mix_ref = self.mix_lst[-1]
                refst_idx = ((len(self.mix_lst)-1) *
                               np.ones(n_mol, dtype='int_'))
            else:
    ...
    def calculate(self):
        ...
        keys = [key for key in keys
                if re.match('(lng|aim_|pm_).*', key)]
        for key in keys:
            tot_res_odct[key] = []
            for idx_job, idx_jobm in enumerate(self.jobmix_idxs):
                assert(idx_job == len(tot_res_odct[key]))
                tot_res_odct[key].append(
                   self.mix_lst[idx_jobm].tot_res[key].copy())
                for idx_mol in range(n_mol):
                    idx_refm = self.refmix_idxs[idx_job, idx_mol]
                    if idx_refm != -1:
                        tot_res_odct[key][idx_job][idx_mol] -= (
                            self.mix_lst[idx_refm].tot_res[key][idx_mol])
        ...
        for key in keys:
            ...
            for idx_job, idx_jobm in enumerate(self.jobmix_idxs):
                ...
                enth_res_odct[key].append(
                    self.mix_lst[idx_jobm].enth_res[key].copy())
                for idx_mol in range(n_mol):
                    idx_refm = self.refmix_idxs[idx_job, idx_mol]
                    if idx_refm != -1:
                        enth_res_odct[key][idx_job][idx_mol] -= (
                            self.mix_lst[idx_refm].enth_res[key][idx_mol])
        ...
        for key in keys:
            ...
            for idx_job, idx_jobm in enumerate(self.jobmix_idxs):
                ...
                comb_res_odct[key].append(
                    self.mix_lst[idx_jobm].comb_res[key].copy())
                for idx_mol in range(n_mol):
                    idx_refm = self.refmix_idxs[idx_job, idx_mol]
                    if idx_refm != -1:
                        comb_res_odct[key][idx_job][idx_mol] -= (
                            self.mix_lst[idx_refm].comb_res[key][idx_mol])
...
class Mixture(object):
    ...
    def __init__(self, x, T, enth, comb):
        ...
        if len(x) != len(enth.mol_lst):
            raise ValueError('Number of mole fractions inconsistent '
...
class COSMORSCombinatorial(object):
    ...
    def calculate(self, x, T):
        ...
        if self.par.comb_term == 'staverman_guggenheim':
            ...
            lng = (np.log(phi_dash_pxi)+1-phi_dash_pxi -
                   self.par.comb_sg_z_coord*0.5 *
                   rel_area*(np.log(buffarr1)+1-buffarr1))
            ...
            lng = (np.log(phi_dash_pxi_scale)+1-phi_dash_pxi_scale -
                   self.par.comb_sg_z_coord*0.5 *
                   rel_area*(np.log(buffarr1)+1-buffarr1))

        else:
            raise ValueError('Unknown combinatorial term '
                             f'{self.par.comb_term}')
...
class COSMORSEnthalpic(object):
    ...
    def add_molecule(self, filepath_lst):
        ...
        self.mol_lst.append(Molecule(filepath_lst,
                                               self.par.qc_program))
        mol = self.mol_lst[-1]
    ...
    def calculate(self, x, T):
        ...
        Gamma, X, n_iter = self._calculate_cosmospace_succ_subst(
            x, int_arr_dct['tau'])
        ...
        if self.par.calculate_contact_statistics_molecule_properties:
            cp_stp_stp = self._calculate_cont_prob_pss(X, Gamma,
                                                       int_arr_dct['tau'])

            aim_dct = self._calculate_av_int_en_mol_aim(cp_stp_stp,
                                                        int_arr_dct)
            result.update(aim_dct)

            ndGamma_dni = self._calculate_ndGamma_dni(x, X, Gamma,
                                                      int_arr_dct['tau'])

            ndpss_dni = self._calculate_ndpss_dni(
                x, X, Gamma, int_arr_dct['tau'], ndGamma_dni)

            pm_dct = self._calculate_part_mol_pm(x, X, Gamma, cp_stp_stp,
                                              int_arr_dct, ndpss_dni)
    ...
    def _calculate_A_hb_E_hb(self, T):
        ...
        del_sigma_hbd_arr[del_sigma_hbd_arr>0] = 0.
        ...
        del_sigma_hba_arr[del_sigma_hba_arr<0] = 0.

        # Convert to 2 dimensional hb delta_sigma*prefactor array
        del_sigma_hbd_mult_hba = (
            del_sigma_hba_arr*del_sigma_hbd_arr.reshape(-1, 1))
        ...
        if buffdb1 > 0:
            ...
            hb_ge_conv_factor = (
                (1/buffdb1)*(1.0-self.par.hb_c_T+2*self.par.hb_c_T*298.15/T))
        else:
            hb_c_at_T = 0.
            hb_ge_conv_factor = 0.
    ...
    def _calculate_cosmospace_succ_subst(self, x, tau):
        ...
        while True:
            ...
            if np.all(np.abs(Gamma_new-Gamma)/Gamma <
                      self.par.cosmospace_conv_thresh):
                converged = True
                break
    ...
    def _calculate_ndGamma_dni(self, x, X, Gamma, tau):
        ...
        for mol in self.mol_lst:
            if len(mol.cosmo_struct_lst) > 1:
                raise ValueError('Gamma derivatives available only for single '
                                 'conformers')
        ...
        N_i_I = np.array([mol.get_segtp_nseg_arr(self.par.a_eff, len(X))
                          for mol in self.mol_lst])
        ...
        if test:
            ...
            for i_mol in range(len(x)):
                ...
                Gamma_var, X_var, n_iter = (
                    self._calculate_cosmospace_succ_subst(x, tau))
    ...
    def _calculate_ndpss_dni(self, x, X, Gamma, tau, ndGamma_dni):
        ...
        if test:
            ...
            N_molecules = len(x)
            pss = self._calculate_cont_prob_pss(
                X, Gamma, tau)
            # Define concentration difference for numerical gradient
            ndpss_dni_NG_lst = []
            n_total = 10000000
            for i_mol in range(len(x)):
                ...
                Gamma_var, X_var, n_iter = (
                    self._calculate_cosmospace_succ_subst(x_var, tau))
                pss_var = self._calculate_cont_prob_pss(
                    X_var, Gamma_var, tau)
                # Calculate with numerical gradient
                ndpss_dni_NG_lst.append(
                    (pss_var-pss)/1*n_total)
    ...
    def _calculate_part_mol_pm(self, x, X, Gamma, pss, int_arr_dct,
                               ndpss_dni):
        """
        ...
        for idxm in range(n_mol):
            buffarr2 = buffflt1*(X.reshape(-1, 1)*ndpss_dni[idxm, :, :] +
            ...
            for int_key in int_keys:
                ...
                pm_dct[new_key][idxm] = (
                    (int_arr_dct[int_key]*buffarr3).sum())
            ...
            for idxm in range(n_mol):
                ...
                for idxs1 in range(len(X)):
                    for idxs2 in range(len(X)):
                        dP_sym_arr_dn_test[idxs1, idxs2] = X[idxs2]*Gamma[idxs1]*Gamma[idxs2]*tau[idxs1, idxs2]*ndX_dni[idxm, idxs1]
                        dP_sym_arr_dn_test[idxs1, idxs2] += X[idxs1]*Gamma[idxs1]*Gamma[idxs2]*tau[idxs1, idxs2]*ndX_dni[idxm, idxs2]
                        dP_sym_arr_dn_test[idxs1, idxs2] += X[idxs1]*X[idxs2]*Gamma[idxs2]*tau[idxs1, idxs2]*ndGamma_dni[idxm, idxs1]
                        dP_sym_arr_dn_test[idxs1, idxs2] += X[idxs1]*X[idxs2]*Gamma[idxs1]*tau[idxs1, idxs2]*ndGamma_dni[idxm, idxs2]
                buffarr2_test = dP_sym_arr_dn_test*N_test
                P_sym_test = np.zeros((len(X), len(X)))
                for idxs1 in range(len(X)):
                    for idxs2 in range(len(X)):
                        P_sym_test[idxs1, idxs2] = X[idxs1]*X[idxs2]*Gamma[idxs1]*Gamma[idxs2]*tau[idxs1, idxs2]
                buffarr3_test = 0.5*(buffarr2_test+P_sym_test*Ni_test)
                pm_A_int_test = 0.
                for idxs1 in range(len(X)):
                    for idxs2 in range(len(X)):
                        pm_A_int_test += int_arr_dct['A_int'][idxs1, idxs2] * buffarr3_test[idxs1, idxs2]
        ...
        if test:
            ...
            for int_key in int_keys:
                ...
                buffval0 = (N_per_n*n_total*0.5*(int_arr_dct[int_key]*
                                                 pss*
                                                 X.reshape(-1, 1)).sum())
                for i_mol in range(len(x)):
                      # Calculate Gammas at numerical gradient temperatures
                    ...
                    X_var = self._calculate_X(x_var)
                    Gamma_var, X_var, n_iter = (
                        self._calculate_cosmospace_succ_subst(
                    ...
                    buffval1 = (N_per_n_var*n_var.sum()*0.5*
                                int_arr_dct[int_key]*
                                cp_stp_stp_var*X_var.reshape(-1, 1)).sum()
                    # Calculate with numerical gradient
                    pm_dct_NG[new_key].append(
                        (buffval1-buffval0)/1)
